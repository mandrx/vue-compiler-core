...
function baseParse(content, options = {}) {
    ...
    let _parseChildren = parseChildren(context, 0 /* DATA */, [])
    let _getSelection = getSelection(context, start);
    let _createRoot = createRoot(_parseChildren, );
...
function generate(ast, options = {}) {
    ...
    //FIV5S Changes - Wrap in try catch block
    push(`try{
        `)
    indent();
    /*---------------------------------------------------*/
...
function genExpression(node, context) {
    ...
    context.push(isStatic ? 
        JSON.stringify(content): 
        content
        //(()=>{try{content}catch{window.console.log('MISSING FUNCTION')}})
         , node);
}
function genInterpolation(node, context) {
    ...
    //genNode(`try{${node.cotent}}catch{window.console.log('Function not found! :: ${node.content}')}`, context);
